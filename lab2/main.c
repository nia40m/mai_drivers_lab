...
/* fops for all users */
static const struct file_operations mai_pipe_fops = {
	.owner   = THIS_MODULE,
	.open    = mai_pipe_open,
...
/* fops for superuser */
static const struct file_operations mai_pipe_fops_superuser = {
	.owner = THIS_MODULE,
	.open  = mai_pipe_open,
...
static int __init mai_pipe_init(void)
{
	...
	status = register_chrdev(MAI_MAJOR_NUMBER,
		MAI_DEVICE_NAME, &mai_pipe_fops);
	if (status < 0)
		return -1;
...
static int mai_pipe_open(struct inode *inode, struct file *file)
{
	...
	if (curr == NULL) {
		curr = kmalloc(sizeof(struct buffer), GFP_KERNEL);
		if (curr == NULL)
			return -1;

		curr->buff = kmalloc(buff_size, GFP_KERNEL);
		if (curr->buff == NULL) {
			kfree(curr);
			return -1;
		}
...
(struct file *file, char *buff, size_t size, loff_t *off)
{
	...
	while (actual_size < size && curr->opened > 1) {
		...
		if (curr->num > 0)
			temp_size =
				(temp_size > curr->num) ? curr->num : temp_size;
		else
			break;
...
(struct file *file, char *buff, size_t size, loff_t *off)
{
	...
	pr_info("Ha-ha, RETARD! YOU CAN'T DO SUCH THING!\n");
...
(struct file *file, const char *buff, size_t size, loff_t *off)
{
	...
	pr_info("USE LINUX KERNEL PIPE, SCALLYWAG!\n");
